...
SELECT mdate, teamname
 FROM game JOIN eteam ON (team1=eteam.id  OR team2=eteam.id)
  WHERE
   eteam.id = team1
    AND coach = 'Fernando Santos'

/* 7. List the player for every goal scored in 
...
SELECT DISTINCT player
  FROM game JOIN goal ON matchid = id 
    WHERE (team1 <> 'GER' AND team2 = 'GER' AND teamid = team1) 
    	OR (team1 = 'GER' AND team2 <> 'GER' AND teamid = team2)
